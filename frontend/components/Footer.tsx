    ...
    <footer className="w-full relative overflow-hidden bg-[#0A0D1E] text-white/80 py-16 mt-0 text-[15px] border-t border-white/10">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10 flex flex-col items-center">
        <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-12 items-start mb-16 px-4">
          ...
          <div className="flex flex-col items-start gap-6">
            <Link href="/" className="flex items-center gap-2">
              <Image src="/TactLink-Logo-core.webp" alt="TactLink" width={32} height={32} className="h-8 w-auto object-contain brightness-0 invert" />
          ...
          <div className="flex flex-col items-start md:items-center gap-6">
            <h3 className="text-white font-bold tracking-widest uppercase text-xs">{t("quickLinks")}</h3>
            <nav className="flex flex-col items-start md:items-center gap-3 text-[14px]">
              <Link href="/" className="hover:text-white hover:translate-x-1 transition-all duration-300">{t("home")}</Link>
              <Link href="/about" className="hover:text-white hover:translate-x-1 transition-all duration-300">{t("about")}</Link>
              <Link href="/digital-namecard" className="hover:text-white hover:translate-x-1 transition-all duration-300">{t("namecard")}</Link>
              <Link href="/terms" className="hover:text-white hover:translate-x-1 transition-all duration-300">{t("terms")}</Link>
              <Link href="/privacy" className="hover:text-white hover:translate-x-1 transition-all duration-300">{t("privacy")}</Link>
            </nav>
          </div>

          {/* Social & Contact */}
          <div className="flex flex-col items-start md:items-end gap-6 md:ml-auto w-full">
            <h3 className="text-white font-bold tracking-widest uppercase text-xs">{t("connect")}</h3>
        ...
        <div className="mt-8 flex flex-col md:flex-row items-center justify-between w-full px-4 text-[13px] text-white/40">
          <div>&copy; {new Date().getFullYear()} <span className="text-white/80 font-bold">TactLink</span>. {t("copyright")}</div>
